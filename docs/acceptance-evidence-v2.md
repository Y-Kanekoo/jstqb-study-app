...
| 公式配分根拠 | `ContentOfficialExamStructureBasisV1`の固定source SHA-256と40問章`8/6/4/11/9/2`・K`8/24/8`からrational演算し、floor合計499、最大の小数剰余、小数剰余同率時は章番号昇順で`100/75/50/138/112/25`、K`100/300/100`へ至る独立3実装golden。basis hashとallocation hashへlossless結合 | Markdown表 | 浮動小数round、source/basis hashなし、小数同率を実行ごとに変更 |
...
| D-01 runtime control matrix | 実DB＋Web/実機で`false + crypto feature OFF => P0 recent-auth可/暗号表示0`、`true + OFFまたは依存不足 => accept/attest/stage/publish全拒否`、`missing/null => fail-closed`、`true + P1完備 => crypto可`の4分岐を照合する。全分岐でglobal suspended DB拒否と配備済み緊急suspend開始RPCを維持する | missingをfalse扱い、true時recent-auth fallback、暗号未達でsuspendまで停止 |
...
- D-04のowner承認factと、承認済み`allocationVersion:1`に対する総数500、章、K、64 LO、single 440、multiple 60の全exact count。公式basis source/hash、40問章/K比率、500倍rational、最大の小数剰余・小数同率時章番号昇順の再計算を含む。D-04未決定ならrelease証跡を作らず未達とする
