...
runtime control `cryptographicReleaseAttestationRequired`はmissing/nullをfalseへdefaultしません。falseかつ暗号feature OFFの時だけP0 recent-authを許可し、trueでfeature OFFまたは依存不足ならpersonal/privateを含むaccept、attest、stage、publishを全拒否します。trueかつP1のcredential lifecycle・署名envelope・rotation/revocation・migration/client/goldenが完備した時だけ暗号経路を許可します。いずれの分岐でもglobal suspended statusのDB拒否と配備済み緊急suspend開始RPCを維持します。
