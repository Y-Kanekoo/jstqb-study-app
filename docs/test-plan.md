...
DB試験はPRごとに必須check `database`でliteral 5 phaseを同じexact migration headへ実行します。(1)`fresh`: 空DBへ全migrationを適用して全`supabase/tests/*.sql`を実行、(2)`origin-main-upgrade`: `origin/main`-shaped schema/fixtureから通常upgrade、(3)`combined-order`: fresh/upgrade両経路のmigration ID/hash/順序・最終schema・RPC署名を照合、(4)`atomic-failure`: 各非atomic境界へ失敗を注入して完全rollback、(5)`production-boundary`: synthetic fixture stable ID/canary/本文/hashのproduction migration/seed/bundle/artifact混入0を検証します。各phaseでDDL/data/migration history/auditの件数・hashと残留0を保存し、一phaseでも欠落・別名・別head・部分適用・fixture混入なら失敗します。RLSや`SECURITY DEFINER` RPCを追加するPRは、匿名・learner・reviewer/admin/serviceの許可/拒否と、非公開データの漏えい防止をpgTAPへ追加します。
...
27. 通常演習の端末Aが回答を確定した後、端末Bの古い`draft.saved`をbarrier同期で到着させる。serverはcanonical attemptへexact一致する`supersededByAttemptId`と`supersededByAttemptHash`を持つ`superseded-by-answer` canonical ACKを返し、draft/attemptを更新しない。端末Bは両値を検証し、ACK保存、outbox `SUPERSEDED`、同itemのpending intent/conflict除去、同じID/hashのcanonical attempt表示を一local transactionで適用する。ID/hashの欠落・差替え・不一致を拒否する。同event replay、ACK受信前、transaction各write間、commit直後のkill/restart、full bootstrap後の再受信でもattempt一件、pending/conflict 0、再送loop 0へ収束する。
...
3. personal/public manifestを別のimmutable bytes/hashにし、publicはparent personal hash、preview、technical/editorial artifactを持ち、旧personal acceptanceを更新しない。
