...
function fixtureBundle(): ProductionBundle {
  ...
  const question: ProductionQuestion = {
    ...
    choices: [
      { id: `${id}-A`, label: 'A', body: '品質情報を提供する', isCorrect: true, explanation: 'リスクと品質に関する情報を意思決定者へ提供できます。' },
      { id: `${id}-B`, label: 'B', body: '無欠陥を証明する', isCorrect: false, explanation: '有限のテストで欠陥が存在しないことを証明できません。' },
      { id: `${id}-C`, label: 'C', body: '担当者を査定する', isCorrect: false, explanation: '担当者の査定はソフトウェアテストの目的ではありません。' },
      { id: `${id}-D`, label: 'D', body: '修正を実装する', isCorrect: false, explanation: '修正の実装は開発やデバッグの活動に分類されます。' },
    ],
